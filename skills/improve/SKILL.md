...
Берёт `./workflow/features/{slug}/plan.md` и `tests.md` фичи в работе, перепроверяет против актуального кода и проектного контекста, находит улучшения, переписывает оба артефакта целиком, обновляет `./workflow/PLAN.md`. Вторая итерация над планом — не замена `plan-feature`, не подготовка к разбивке.
...
5. **Применимость по статусу фичи в `./workflow/PLAN.md`:**
   ...
   - `[ ]` (план не создан) → отказ, направить на `plan-feature`.
...
- **slug передан, статус `[ ]`** → стоп: «план не создан, запусти `plan-feature` для slug={slug}».
...
- **slug не передан** → собери фичи со статусом `[-]` из `./workflow/PLAN.md`. Одна — бери её. Несколько — `AskUserQuestion` со списком вариантов. Ни одной — стоп: «нет фич для улучшения. Сначала создайте фичу (`feature`) и спланируйте (`plan-feature`)».
...
Улучшений нет → короткий отчёт «план в порядке, готов к разбивке через `plan-to-tasks`». Дальше не идём.
...
- следующий шаг (обычно `plan-to-tasks` для разбивки на задачи).
