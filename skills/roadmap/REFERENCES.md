...
- Оценка текущего состояния полезна перед дорожной картой.
...
1. Прочитай `./workflow/PROJECT.md`, существующие `./workflow/VISION.md`, `./workflow/ROADMAP.md` и `./workflow/PLAN.md`, если они есть.
2. Подними минимальный контекст из кода и документации только если без него непонятны продукт, пользователи или текущие возможности.
3. Выдели текущее назначение проекта, аудиторию, проблему, ограничения и фактическое состояние.
4. Если намерения пользователя неясны, задай 1-3 вопроса о цели, аудитории, горизонте и приоритетах.
5. Сформулируй `VISION.md`: смысл, пользователи, проблема, принципы, non-goals, признаки успеха.
6. Сформулируй `ROADMAP.md`: цели, outcomes, горизонты, приоритеты, зависимости, риски, assumptions, not-now.
7. Используй легкую prioritization lens: value, strategic fit, effort, risk, dependencies, confidence.
8. Отдели уверенные направления от гипотез.
9. Не превращай roadmap в список фич: у каждого направления должен быть outcome или rationale.
10. В `./workflow/PLAN.md` добавь только служебные roadmap-хвосты, если нужны дополнительные решения.
11. Запиши документы как текущее состояние проекта, без истории изменений и сравнений с прежними версиями.
