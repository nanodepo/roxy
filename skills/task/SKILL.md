...
feature status `[+]` in `./workflow/PLAN.md`.

Use after `planning` or `improve`. Read feature brief, plan, design if present.
Cut work into tasks sized for sequential `implement`. May rewrite `plan.md` as
phase/task map when downstream work becomes clearer.

Task files feed `implement`; `test` / `docs` may read them for real scope. Own
only `NN-task.md` and navigational normalization of `plan.md`. Do not write
code, `tests.md`, or other `./workflow/` docs beyond feature status.
...
- Own only `./workflow/features/{slug}/NN-task.md`, navigational map in
...
- Do not change `plan.md` substance. Rewrite only to map phases to created
  `NN-task.md` files.
...
If `NN-task.md` files already exist, treat as update: rebuild task set from
current `plan.md`, not append old files.
...
Use `plan.md` as scope authority. Do not add work absent from plan.
...
`implement` from `plan.md` or `feature.md`, and still update
`./workflow/PLAN.md` per Step 8.
...
- whether `plan.md` should become phase/task map and what map looks like;
...
If phase/task map helps downstream work, rewrite
`./workflow/features/{slug}/plan.md` to list phases and point each phase at
`NN-task.md` files. Keep plan content; turn into map, do not strip substance.
If existing `plan.md` already clear, leave as-is.
...
- `plan.md` map, if rewritten, points cleanly at task files.
